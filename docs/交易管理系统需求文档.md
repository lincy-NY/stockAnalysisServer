...
**通知中心**：
- 点击通知图标，弹出下拉列表（固定宽度400px，最大高度500px，超出滚动）
- 标题栏："消息通知"（右侧"全部标为已读"按钮）
- 显示所有未读消息（按时间倒序）
- 每条消息显示：
  ```
  ┌─────────────────────────────┐
  │ [平安银行] 触发卖点          │
  │ 类型：s2（跌破多空线）        │
  │ 日期：2026-06-26  价格：10.50元│
  │              [查看] [已读]    │
  └─────────────────────────────┘
  ```
- 操作按钮：
  - [查看详情]：跳转到持仓详情页（不影响已读状态）
  - [已读]：将消息状态改为已读，从列表中移除
        ...
        for signal_type, signal_date in signals.items():
            if signal_date:  # 信号被触发
                ...
                cursor.execute("""
                    ...
                    WHERE position_id = %s AND alert_type = %s AND alert_date = %s
                """, (pos['id'], signal_type, signal_date))

                if not cursor.fetchone():
                    # 插入新的未读提示
                    cursor.execute("""
                        INSERT INTO stock_alert
                        (position_id, ts_code, stock_name, alert_type, alert_date, trigger_desc, status)
                    ...
                    conn.commit()
                    print(f"  💡 {ts_code} {pos['stock_name']} 触发{signal_type}信号，已添加未读提示")
```
