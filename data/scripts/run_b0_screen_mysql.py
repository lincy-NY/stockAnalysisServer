...
B0点选股筛选脚本（MySQL版）
条件：
1. 收盘价 > big_line（股价高于多空线）
2. 当日short_line上穿big_line（短期线金叉多空线）
"""
...
def get_stock_data(conn, ts_code, table='stock_daily'):
    """从MySQL获取股票数据"""
...
def screen_stock(conn, ts_code, table='stock_daily'):
    """筛选单只股票"""
    df = get_stock_data(conn, ts_code, table)
    ...
    # 即：当日 short_line > big_line 且 前一日 short_line <= big_line
...
def get_all_stock_codes(conn, table='stock_daily'):
...
def main():
    print("=" * 60)
    print(f"B0点选股筛选(MySQL) - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    if results:
        ...
        for r in results:
            ...
            dist_short = (r['short_line'] - r['close']) / r['close'] * 100
            dist_pct = (r['close'] - r['big_line']) / r['big_line'] * 100
            sql = '''
                INSERT IGNORE INTO stock_select_daily
                (strategy, screen_date, ts_code, trade_date, close, low_price, big_line, short_line, dist_pct, dist_short, vol, amount)
                VALUES ('b0', %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            '''
            cursor.execute(sql, (
                today, r['code'], trade_date_fmt,
                r['close'], r['low'], r['big_line'], r['short_line'], round(dist_pct,4), round(dist_short,4), r['vol'], r['amount']
            ))
        conn.commit()
        print(f"\n💾 结果已保存到统一表 stock_select_daily (b0)")
        
        # 保存Excel
        output_file = OUTPUT_DIR / f'b0_result_{today_fmt}.xlsx'
        df_result = pd.DataFrame(results)
        df_result.to_excel(output_file, index=False, sheet_name='B0日线选股')
