...
def fetch_by_date(pro, trade_date):
    """按日期获取全市场数据"""
    try:
    ...
    for trade_date in dates:
        ...
        for _, row in df.iterrows():
            ...
            rows.append((
                ...
                to_mysql_val(row['amount']), None, None, None, None, None, None
            ))
            updated_codes.add(row['ts_code'])

        sql = """INSERT INTO stock_daily
            (ts_code, trade_date, open, high, low, close, pre_close,
             change_val, pct_chg, vol, amount, short_line, big_line, kdj_j, macd_dif, macd_dea, macd_bar)
    ...
    try:
        ...
        for _, row in df.iterrows():
            if pd.isna(row.get('short_line')):
                continue
            upd_cur.execute("""
                UPDATE stock_daily SET short_line=%s, big_line=%s, kdj_j=%s,
...
def calc_indicators_parallel(ts_codes):
    """多线程并行计算技术指标"""
    total = len(ts_codes)
    print(f"\n需计算指标: {total} 只股票（{INDICATOR_WORKERS}线程并行）")
...
def main():
    ...
    print(f"  本次更新 {len(updated_codes)} 只股票")
