...
def get_pending_signals(conn):
    ...
    cursor.execute("""
        SELECT id, ts_code, low_price, big_line, screen_date
        FROM stock_select_weekly
        WHERE strategy = 'b1' AND is_track = 1
...
def update_track_record(conn, relate_id, ts_code, low_price):
    ...
    target_price = float(low_price) * 0.99

    prev = get_prev_signals(conn, relate_id)
    prev_s1 = prev['s1'] if prev else None
    prev_s2 = prev['s2'] if prev else None
    prev_s3 = prev['s3'] if prev else None

    s1 = prev_s1 or (track_date if close < target_price else None)
    s2 = prev_s2 or (track_date if close < big_line else None)
    s3 = prev_s3 or (track_date if short_line < big_line else None)
...
def main():
    print(f"B1周线追踪 - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    for sig in signals:
        relate_id = sig['id']
        ts_code = sig['ts_code']
        low_price = sig['low_price']

        result = update_track_record(conn, relate_id, ts_code, low_price)
        if result:
            tracked += 1
            if result['s3']:
                end_tracking(conn, relate_id)
                ended += 1
                print(f"  {ts_code} 触发s3，结束追踪")
